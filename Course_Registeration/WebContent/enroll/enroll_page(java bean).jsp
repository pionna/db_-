...
<%
	Vector v = null;
	v = EnrollMgr.getEnrollList(studentID, year, semester);
	for(int i=0; i<v.size(); i++){
		Enroll en = (Enroll)v.elementAt(i);
	%>
		<tr><td><%=en.getC_id()%></td><td><%=en.getC_name()%></td><td><%=en.getC_id_no()%></td><td><%=en.getC_major()%></td><td><%=en.getC_unit()%></td>
		<td><%=en.getP_name()%></td><td><%=en.getT_day()%> <%=en.getT_time()%></td><td><%=en.getT_room()%></td>
		<td><a target="popup" onclick="window.open('../search/student_list.jsp?c_id=<%=en.getC_id()%>&c_id_no=<%=en.getC_id_no()%>&year=<%=year%>&semester=<%=semester%>','학생목록','width=800,height=400')" style="color:#0174DF;"><%=en.getStudentNum()%>/<%=en.getT_max()%></a></td>
		<td><a href="./delete.jsp?year=<%=yearStr%>&semester=<%=semesterStr%>&c_id=<%=en.getC_id_no()%>&c_id_no=<%=en.getC_id_no()%>"><%=validCancel%></a></td></tr>
<%}
%> 
</table>
<table>
	<tr><td width= "65%"></td><td>총수강과목: <%= Enroll.totalEnrolledClass %></td><td>총수강학점: <%= Enroll.totalEnrolledUnit %></td></tr>
</table>
